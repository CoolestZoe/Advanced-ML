...
if __name__ == '__main__':
    ...
    def rotate_all(all_img,angle):
        for x in all_img:
            all_img[x]=skimage.transform.rotate(all_img[x],angle)
    ...
    def train_net():
        ...
        for a in range(1):
            if (a>0):
                rotate_all(X_DICT_TRAIN,30)
                rotate_all(Y_DICT_TRAIN,30)

            xx_train,yy_train =get_patches(X_DICT_TRAIN, Y_DICT_TRAIN, n_patches=TRAIN_SZ, sz=PATCH_SZ)
        
            x_train.extend(xx_train)
